...
class BasicBlock_plus(torch.nn.Module):
    def __init__(self):
        super(BasicBlock_plus, self).__init__()
...
# Define ISTA-Net
class ISTANet_plus(torch.nn.Module):
    def __init__(self, LayerNo):
        super(ISTANet_plus, self).__init__()
        onelayer = []
        self.LayerNo = LayerNo

        for i in range(LayerNo):
            onelayer.append(BasicBlock_plus())
...
class BasicBlock_MRI(torch.nn.Module):
    def __init__(self):
        super(BasicBlock_MRI, self).__init__()
...
class ISTANet_MRI(torch.nn.Module):
    def __init__(self, LayerNo):
        super(ISTANet_MRI, self).__init__()
        onelayer = []
        self.LayerNo = LayerNo
        self.fft_forback = FFT_Mask_ForBack()

        for i in range(LayerNo):
            onelayer.append(BasicBlock_MRI())
...
class BasicBlock_MRI_plus(torch.nn.Module):
    def __init__(self):
        super(BasicBlock_MRI_plus, self).__init__()
    ...
    def forward(self, x, fft_forback, PhiTb, mask):
        ...
        # x_st = torch.mul(torch.sign(x_forward), F.relu(torch.abs(x_forward) - self.soft_thr))
        # x_ft = firm_thresh(x_forward, self.lambda_ft, self.gamma)
        # x_scad = scad_thresh(x_forward, self.lambda_scad, self.a_scad)
        # theta = 1/3
        # x = (theta * x_st) + (theta * x_ft) + (theta * x_scad)
...
# Define ISTA-Net-plus
class ISTANet_MRI_plus(torch.nn.Module):
    def __init__(self, LayerNo):
        super(ISTANet_MRI_plus, self).__init__()
        onelayer = []
        self.LayerNo = LayerNo
        self.fft_forback = FFT_Mask_ForBack()

        for i in range(LayerNo):
            onelayer.append(BasicBlock_MRI_plus())
