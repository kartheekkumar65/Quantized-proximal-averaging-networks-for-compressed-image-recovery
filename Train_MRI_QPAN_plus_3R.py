# This file implements PAN plus (3R) model for MRI reconstruction
...
#%%     Architecture of the PAN plus 3R model
...
#   PAN plus 3R model 
class PAN_plus_3R(torch.nn.Module):
    def __init__(self, LayerNo):
        super(PAN_plus_3R, self).__init__()
        onelayer = []
...
for epoch_i in range(start_epoch, end_epoch):
    ...
    if epoch_i % 3 == 0:
        ...
        # save the models
        torch.save(model.state_dict(), "./%s/net_params_bits_%s_3R_epoch_%d.pkl" % (model_dir, str(bits), epoch_i))  # save only the parameters
